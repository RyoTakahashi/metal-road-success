...
export const BG_SRC: Record<BgKey, string> = {
  studio: `${base}assets/bg/studio.v1.webp`,
  street: `${base}assets/bg/street.v1.webp`,
  venueSmall: `${base}assets/bg/venue_small.v1.webp`,
  venueBig: `${base}assets/bg/venue_big.v1.webp`,
  backstage: `${base}assets/bg/backstage.v1.webp`,
};

export type Mood = "normal" | "fired" | "happy" | "sad";

// Character version per member. Bump when the look changes (see docs/assets.md);
// the filename is `{id}.v{version}.{mood}.webp`, or with an evolution infix
// `{id}.v{version}.{evo}.{mood}.webp` (e.g. ryo.v2.goth.fired.webp). Sprites are
// WebP (q82) — ~8× smaller than the source PNGs, for fast web/itch.io loads.
...
export const charSrc = (member: string, mood: Mood = "normal"): string => {
  ...
  if (!EVO_ART_READY || !currentEvo) return `${base}assets/chars/${lc}.v${v}.${mood}.webp`;
  ...
  return `${base}assets/chars/${lc}.v${v}.${currentEvo}.${m}.webp`;
};
